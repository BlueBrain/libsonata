...
class IoWriter {
  ...
  public:
    ...
    static std::unique_ptr<IoWriter> create_io_writer(IoWriter::Kind kind, const std::string& report_name);
    ...
    virtual void write(double* buffer, int steps_to_write, int total_steps, int total_compartments) = 0;

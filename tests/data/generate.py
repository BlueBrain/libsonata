...
def write_soma_report(filepath):
    ...
    with h5py.File(filepath, 'w') as h5f:
        ...
        gpop_all.create_dataset('data', data=d1, dtype=np.float32)
        gmapping = h5f.create_group('/report/' + population_names[0] + '/mapping')

        gmapping.create_dataset('node_ids', data=node_ids, dtype=np.uint64)
...
def write_spikes(filepath):
    ...
    with h5py.File(filepath, 'w') as h5f:
        ...
        gpop_spikes2.create_dataset('timestamps', data=timestamps_base, dtype=np.double)
